...
cpdef inline void clip(double[:] arr, double[:] out, double low_value, double high_value):
...
cpdef void clip_norm(double[:] arr, double[:] out, double low_value, double high_value):
    cdef ssize_t x, nx = arr.shape[0]
    if &arr[0] != &out[0]:
        out[...] = arr[...]
    with nogil:
